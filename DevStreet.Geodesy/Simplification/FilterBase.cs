...
namespace DevStreet.Geodesy.Simplification
{
    ...
    public class FilterBase
    {
        ...
        internal static void AddLastIfNotContains<T>(ICollection<T> collection, IEnumerable<T> items) where T : class
        {
            if (items == null)
            {
        ...
        internal static IEnumerable<SequentialCoordinate> ToEnumerableSequentialCoordinate(IEnumerable<ICoordinate> items)
        {
            if (items == null)
            {
                throw new ArgumentNullException(nameof(items), "The argument cannot be null.");
